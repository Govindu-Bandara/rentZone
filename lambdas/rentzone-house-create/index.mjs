...
// Extract city from title (e.g., "Luxury House in Colombo 05" => "Colombo 05")
function extractCityFromTitle(title) {
  ...
  if (match && match[1]) {
    ...
    return extractedCity.length > 0 ? extractedCity : null;
  }
...
export const handler = async (event) => {
  ...
  try {
    ...
    // Determine mainImage: use provided mainImage or default to first image
    const selectedMainImage = mainImage || (parsedImages.length > 0 ? parsedImages[0] : null);
    
    // Extract city from title if present
    const titleExtractedCity = extractCityFromTitle(title);
    const finalCity = titleExtractedCity || location.city;
    ...
    await logSystemActivity(db, 'INFO', 'Listing', 
      ...
      ipAddress,
      {
        ...
        city: finalCity,
        mainImage: selectedMainImage,
        extractedCity: titleExtractedCity
      }
    );
